...
# Gestione configurazione tabelle (Classi, Sedi, Modalità)
def carica_config():
    default_config = {
...
def salva_dati(df_to_save):
    ...
    try:
        ...
        if worksheet is None:
            st.error(
                "Impossibile connettersi a Google Sheets. Verifica i Secrets."
            )
            return
...
with tab3:
    ...
    if not df.empty:
        ...
        st.markdown(
            f"Risultati filtrati: **{len(df_report)}** attività | Ore totali:"
            f" **{ore_totali:.2f} ore**"
        )
        ...
        if not df_report.empty:
            ...
            # Generazione dei riquadri personalizzati in stile card a schermo
            for _, row in df_report.iterrows():
                data_formattata = (
