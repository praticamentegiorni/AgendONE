...
st.markdown(
    ...
    /* Card stilizzate per RegistrOne */
    .stCard {
...
def calcola_ore(ora_inizio, ora_fine):
  ...
  try:
    ...
    try:
      ...
      return 0.0
  diff = (
      datetime.datetime.combine(datetime.date.min, t_f.time())
      - datetime.datetime.combine(datetime.date.min, t_i.time())
...
def genera_pdf_report(df_report):
  try:
    ...
    ente_header_style = ParagraphStyle(
        "EnteHeaderStyle",
    ...
    td_summary_style = ParagraphStyle(
        "TDSummary",
        parent=styles["Normal"],
        fontSize=8,
        fontName="Helvetica",
    ...
    style_subtot_classe = ParagraphStyle(
        "SubTotClasse",
        parent=styles["Normal"],
        alignment=2,
        fontSize=8,
    ...
    style_subtot_val_classe = ParagraphStyle(
        "SubTotValClasse",
        parent=styles["Normal"],
        fontSize=8,
        fontName="Helvetica-Bold",
    ...
    elements.append(
        Paragraph("Report Attività e Riepilogo Ore - AgendOne", title_style)
    )
    elements.append(Spacer(1, 6))

    df_validi = (
        df_report[df_report["Escludi_Conteggio"] != True]
        if "Escludi_Conteggio" in df_report.columns
        else df_report
    )

    elements.append(
        Paragraph(
            "Riepilogo Totale Parziali per Ente di Appartenenza",
            subtitle_style,
        )
    )
    if (
        not df_validi.empty
        and "Ente" in df_validi.columns
        and "Ore" in df_validi.columns
    ):
      df_summary_ente = df_validi.groupby("Ente")["Ore"].sum().reset_index()
      summary_ente_data = [[
          Paragraph("Ente di Appartenenza", th_style),
          Paragraph("Ore Totali Parziali", th_style),
      ]]

      for _, row in df_summary_ente.iterrows():
        summary_ente_data.append([
            Paragraph(
                str(row["Ente"]) if row["Ente"] else "Non Specificato",
                td_summary_style,
            ),
    ...
    elements.append(
        Paragraph(
            "Elenco Dettagliato Attività Raggruppate per Ente e Classe",
            subtitle_style,
        )
    )
    ...
    if not df_report.empty:
      ...
      for ente_nome, classi_dict in gruppi_ente_classe.items():
        elements.append(
            Paragraph(f"Ente: <b>{ente_nome}</b>", ente_header_style)
        )
        ...
        for classe_nome in sorted(classi_dict.keys()):
          lista_attivita = sorted(
              ...
              key=lambda r: (
                  (
                      parse_data_italiana(r.get("Data", ""))
                      if pd.notnull(parse_data_italiana(r.get("Data", "")))
          ...
          for row in lista_attivita:
            ...
            det_data.append([
                ...
                Paragraph(
                    f"{row.get('Orario Inizio', '')} -"
                    f" {row.get('Orario Fine', '')}",
                    cur_td_style,
                ),
          ...
          det_data.append([
              Paragraph(
                  f"<b>Totale parziale ({classe_nome}):</b>",
                  style_subtot_classe,
              ),
              "",
              "",
              ...
              "",
              Paragraph(
                  f"<b>{totale_ore_classe:.2f}h</b>", style_subtot_val_classe
              ),
          ])
          table_styles.append(("SPAN", (0, row_idx), (5, row_idx)))
          table_styles.append(
              ("BACKGROUND", (0, row_idx), (-1, row_idx), colors.HexColor("#e6f2ff"))
          )
          row_idx += 1

        det_data.append([
            Paragraph(
                f"<b>Totale Ore Parziali ({ente_nome}):</b>",
                ParagraphStyle(
                    "SubTot",
                    parent=styles["Normal"],
                    alignment=2,
...
def get_gspread_client_and_sheet(nome_foglio="Foglio1"):
  try:
    ...
    st.error(
        f"Errore durante la connessione a Google Sheets ('{nome_foglio}'): {e}"
    )
    return None
...
def sincronizza_google_calendar(azione, dati_evento, evento_id_esistente=None):
  try:
    ...
    if azione != "elimina":
      ...
      body = {
          "summary": (
              f"Lezione/Impegno: [{dati_evento.get('Ente', '')}]"
              f" {dati_evento['Classe']} ({dati_evento['Modalità']})"
          ),
          "location": str(dati_evento["Sede"]),
          "description": (
              f"Ente: {dati_evento.get('Ente', '')}\nNote:"
              f" {dati_evento.get('Note', '')}\n{dati_evento.get('Appunto_Multiplo', '')}\nGestito"
              " da AgendOne"
          ),
          "start": {
    ...
    if azione == "crea":
      ...
      try:
        service.events().update(
            calendarId=calendar_id, eventId=evento_id_esistente, body=body
        ).execute()
      ...
      try:
        service.events().delete(
            calendarId=calendar_id, eventId=evento_id_esistente
        ).execute()
      except HttpError as err:
        if err.resp.status != 404:
          raise err
...
def carica_config():
  ...
  try:
    ...
    for row in all_values[1:]:
      ...
      if (
          idx_modalita != -1
          and len(row) > idx_modalita
          and row[idx_modalita].strip()
      ):
...
def carica_dati():
  ...
  try:
    ...
    if "Data" in df.columns:
      ...
      df.loc[mask_valid, "Data"] = df.loc[mask_valid, "Data_dt"].dt.strftime(
          "%Y-%m-%d"
      )
    ...
    if "Svolto" not in df.columns:
      ...
      df["Svolto"] = df["Svolto"].apply(
          lambda x: (
              True if str(x).lower() in ["true", "1", "yes", "vero", "on"] else False
          )
      )
    ...
    if "Escludi_Conteggio" not in df.columns:
      ...
      df["Escludi_Conteggio"] = df["Escludi_Conteggio"].apply(
          lambda x: (
              True if str(x).lower() in ["true", "1", "yes", "vero", "on"] else False
          )
      )
    ...
    if "Reminder_Minuti" not in df.columns:
      ...
      df["Reminder_Minuti"] = (
          pd.to_numeric(df["Reminder_Minuti"], errors="coerce")
          .fillna(240)
          .astype(int)
      )
    ...
    if "Ore" not in df.columns or df["Ore"].isna().all():
      df["Ore"] = df.apply(
          lambda r: calcola_ore(r.get("Orario Inizio"), r.get("Orario Fine")),
          axis=1,
      )
...
# ================= FUNZIONE REGISTRI DI CLASSE (EX STREAMLIT_APP.PY) =================
def mostra_registri_di_classe():
  ...
  if dati:
    ...
    st.success("Dati caricati con successo da Google Sheets!")
    
    col_reg1, col_reg2, col_reg3 = st.columns(3)
    with col_reg1:
      st.metric("Classi Registrate", len(dati["classi"]))
    with col_reg2:
      st.metric("Materie", len(dati["materie"]))
    with col_reg3:
      st.metric("Totale Alunni", len(dati["alunni"]) if dati["alunni"] is not None else 0)

    if dati["alunni"] is not None and not dati["alunni"].empty:
      st.markdown("### Elenco Alunni")
      st.dataframe(dati["alunni"], use_container_width=True)
...
with tab1:
  ...
  tipo_inserimento = st.radio(
      ...
      options=["Inserimento Normale", "Inserimento Multiplo"],
      horizontal=True,
  )
  ...
  with st.form("form_orario", clear_on_submit=True):
    ...
    with col_t0:
      ente = st.selectbox(
          "Ente",
          options=opts_enti if opts_enti else [""],
          index=0 if opts_enti else 0,
          key="sel_ente",
      )
      nuovo_ente_libero = st.text_input(
          "O digita nuovo ente:", placeholder="Se non è in elenco...", key="lib_ente"
      )
    with col_t1:
      classe = st.selectbox(
          "Classe",
          options=opts_classi if opts_classi else [""],
          index=0 if opts_classi else 0,
          key="sel_classe",
      )
      nuova_classe_libera = st.text_input(
          "O digita nuova classe:", placeholder="Se non è in elenco...", key="lib_classe"
      )
    with col_t2:
      sede = st.selectbox(
          "Sede",
          options=opts_sedi if opts_sedi else [""],
          index=0 if opts_sedi else 0,
          key="sel_sede",
      )
    ...
    with col_t3:
      modalita = st.selectbox(
          "Modalità",
          options=opts_modalita if opts_modalita else [""],
          index=0 if opts_modalita else 0,
          key="sel_mod",
      )
      nuovo_mod_libero = st.text_input(
          "O digita nuova modalità:", placeholder="Se non è in elenco...", key="lib_mod"
      )

    st.selectbox(
        "Avviso / Promemoria Calendar (Disabilitato)",
        options=["Funzione temporaneamente disabilitata"],
        index=0,
        disabled=True,
    )
    st.caption(
        "Nota: La modifica dell'orario di notifica è momentaneamente"
        " disabilitata."
    )
    minuti_scelti = 240

    svolto_iniziale = st.checkbox("Impegno già svolto", value=False)
    escludi_conteggio_iniziale = st.checkbox(
        "Escludi dal conteggio ore", value=False
    )
    ...
    if tipo_inserimento == "Inserimento Multiplo":
      ...
      st.caption(
          "Compila le righe desiderate inserendo l'intervallo di orario e"
          " selezionando una classe o inserendo un testo libero."
      )

      righe_multiplo_lista = []
      opts_classi_multi = [""] + config.get("classi", [])
      for i in range(8):
        rc1, rc2, rc3, rc4, rc5_sel, rc5_txt = st.columns(
            [0.45, 0.45, 0.45, 0.45, 2.1, 2.1]
        )
        with rc1:
          m_ora_i = st.selectbox(
              f"DaO{i+1}",
        ...
        with rc5_sel:
          m_classe_sel = st.selectbox(
              f"Classe{i+1}",
              options=opts_classi_multi,
              index=0,
              key=f"m_classe_sel_{i}",
              label_visibility="collapsed",
          )
        with rc5_txt:
          m_testo_libero = st.text_input(
              f"Testo{i+1}",
              placeholder=f"O testo libero {i+1}...",
              key=f"m_testo_libero_{i}",
              label_visibility="collapsed",
          )

        val_testo_riga = (
            m_testo_libero.strip() if m_testo_libero.strip() else m_classe_sel
        )
        if val_testo_riga:
          orario_slot_str = (
              f"{m_ora_i:02d}:{m_min_i:02d}-{m_ora_f:02d}:{m_min_f:02d}"
          )
    ...
    submit_button_label = (
        "Salva Inserimento Multiplo"
        if tipo_inserimento == "Inserimento Multiplo"
        else "Salva Attività"
    )
    submit_button = st.form_submit_button(
        label=submit_button_label, use_container_width=True
    )

    if submit_button:
      if orario_inizio_str >= orario_fine_str:
        st.error(
            "L'orario di inizio non può essere successivo o uguale all'orario"
            " di fine."
        )
      else:
        ...
        codice_univoco_generato = data_selezionata.strftime(
            "%d%m%Y"
...
with tab2:
  ...
  st.markdown(
      "Gestisci gli elenchi a tendina per l'inserimento rapido delle attività."
      " Il sistema impedisce automaticamente l'inserimento di voci duplicate."
  )
  ...
  def gestisci_sezione_combo(titolo_sezione, chiave_config):
    ...
    with col_sel:
      voce_selezionata = st.selectbox(
          ...
          options=["-- Seleziona per modificare/eliminare --"]
          + sorted(lista_corrente),
    ...
    with st.form(key=f"form_add_{chiave_config}", clear_on_submit=True):
      ...
      with c_in1:
        nuova_voce = st.text_input(
            f"Aggiungi nuovo elemento a {titolo_sezione}",
            placeholder=esci_placeholder(titolo_sezione),
            label_visibility="collapsed",
        )
      with c_in2:
        btn_aggiungi = st.form_submit_button(
            "Aggiungi", use_container_width=True
        )
    ...
    if voce_selezionata != "-- Seleziona per modificare/eliminare --":
      ...
      with st.form(key=f"form_edit_{chiave_config}"):
        ...
        with c_ed1:
          valore_modificato = st.text_input(
              "Rinomina voce",
              value=voce_selezionata,
              label_visibility="collapsed",
          )
        with c_ed2:
        ...
        with c_ed3:
          btn_elimina = st.form_submit_button(
              "Elimina", use_container_width=True, type="primary"
          )
        ...
        if btn_salva_mod:
          ...
          elif valore_pulito.lower() != voce_selezionata.lower() and any(
              v.lower() == valore_pulito.lower() for v in lista_corrente
          ):
  ...
  def esci_placeholder(t):
    if "Enti" in t:
      return "Es. Scuola Bufalini..."
    if "Classi" in t:
      return "Es. Classe 4A..."
    if "Sedi" in t:
      return "Es. Aula Magna..."
    return "Es. Presenza / Online..."

  def restituisci_messaggio_duplicato(t, nome):
    return (
        f"Attenzione: '{nome}' è già presente nell'elenco degli {t.lower()}."
        if t == "Enti"
        else (
            f"Attenzione: '{nome}' è già presente nell'elenco delle"
            f" {t.lower()}."
        )
    )
...
with tab3:
  ...
  if not df.empty:
    ...
    if "Data" in df_vis.columns:
      ...
      df_vis["Mese"] = df_vis["Data_dt"].apply(
          lambda dt: traduci_mese(dt.strftime("%B")).capitalize()
          if pd.notnull(dt)
          else ""
      )
      df_vis["Data"] = df_vis["Data_dt"].dt.strftime("%d/%m/%Y").fillna(
          df_vis["Data"]
      )

      df_vis["Ore"] = df_vis.apply(
          lambda r: calcola_ore(r.get("Orario Inizio"), r.get("Orario Fine")),
          axis=1,
      )
      df_vis = df_vis.sort_values(
          by=["Data_dt", "Orario Inizio"], ascending=[True, True]
      )
    ...
    filtro = st.text_input(
        "Cerca rapidamente nell'archivio:",
    ...
    df_mostra_visibile = df_mostra.drop(
        columns=[c for c in colonne_da_nascondere if c in df_mostra.columns]
    )
    ...
    def colora_righe_tabella(row):
      ...
      if svolto:
        return [
            "background-color: #2b2b2b; color: #7f7f7f; text-decoration:"
            " line-through"
        ] * len(row)
    ...
    df_editato = st.data_editor(
        ...
        hide_index=True,
        column_config={
            "Selezione": st.column_config.CheckboxColumn(
                "Sel.", required=True, width="small"
            ),
            "Svolto": st.column_config.CheckboxColumn(
                "Svolto", required=True, width="small"
            ),
            ...
            "Ore": st.column_config.NumberColumn(
                format="%.2f h", disabled=True, width="small"
            ),
        },
    )

    ore_totali_archivio = (
        df_mostra[df_mostra["Escludi_Conteggio"] != True]["Ore"].sum()
        if "Ore" in df_mostra.columns
        else 0.0
    )
    st.info(
        "**Totale ore (visualizzate in archivio, escluse quelle flaggate):**"
        f" {ore_totali_archivio:.2f} ore"
    )
    ...
    for _, riga_ed in df_editato.iterrows():
      idx_orig = (
          int(riga_ed["ID"])
          if "ID" in df_editato.columns
          else int(
              df_mostra[df_mostra.index == riga_ed.name]["ID"].values[0]
          )
      )
      ...
      if (
          ...
          or df.loc[idx_orig, "Escludi_Conteggio"] != val_nuovo_escluso
      ):
        df.loc[idx_orig, "Svolto"] = val_nuovo_svolto
    ...
    righe_selezionate = (
        df_mostra[df_editato["Selezione"] == True]["ID"].tolist()
        if "Selezione" in df_editato.columns and "ID" in df_mostra.columns
        else []
    )
    ...
    with col_act1:
      if st.button(
          "Elimina Selezionati", type="primary", use_container_width=True
      ):
        if righe_selezionate:
          for r_idx in righe_selezionate:
            cal_id_esistente = (
                str(df.loc[r_idx, "Calendar_ID"])
                if "Calendar_ID" in df.columns
                else ""
            )
            if cal_id_esistente and cal_id_esistente.lower() not in [
                "nan",
                "none",
                "",
            ]:
    ...
    with col_act2:
      if st.button("Duplica Selezionato", use_container_width=True):
        if len(righe_selezionate) == 1:
          ...
          if (
              "Note" in nuova_riga
              and pd.notnull(nuova_riga["Note"])
              and str(nuova_riga["Note"]).strip() != ""
          ):
          ...
          st.warning(
              "Seleziona un solo appuntamento alla volta per la duplicazione"
              " rapida."
          )
        else:
      ...
      with st.form("form_modifica_multipla"):
        ...
        with c_mo1:
          mod_ora_i = st.selectbox(
              "Ora Inizio",
              options=list(range(0, 24)),
              index=h_def_i if h_def_i in range(0, 24) else 9,
          )
        with c_mo2:
          mod_min_i = st.selectbox(
              "Minuti Inizio",
              options=list(range(0, 60)),
              index=m_def_i if m_def_i in range(0, 60) else 0,
          )
        with c_mo3:
          mod_ora_f = st.selectbox(
              "Ora Fine",
              options=list(range(0, 24)),
              index=h_def_f if h_def_f in range(0, 24) else 18,
          )
        with c_mo4:
          mod_min_f = st.selectbox(
              "Minuti Fine",
              options=list(range(0, 60)),
              index=m_def_f if m_def_f in range(0, 60) else 0,
          )
        ...
        idx_ente = (
            enti_esistenti.index(val_ente_corrente)
            if val_ente_corrente in enti_esistenti
            else 0
        )
        mod_ente_sel = st.selectbox(
            "Ente",
            options=enti_esistenti if enti_esistenti else [""],
            index=idx_ente if enti_esistenti else 0,
            key="mod_sel_ente",
        )
        mod_ente_libero = st.text_input(
            "O digita nuovo ente (Modifica):",
        ...
        idx_classe = (
            classi_esistenti.index(val_classe_corrente)
            if val_classe_corrente in classi_esistenti
            else 0
        )
        mod_classe_sel = st.selectbox(
            "Classe",
            options=classi_esistenti if classi_esistenti else [""],
            index=idx_classe if classi_esistenti else 0,
            key="mod_sel_classe",
        )
        mod_classe_libera = st.text_input(
            "O digita nuova classe (Modifica):",
            placeholder="Se non è in elenco...",
            key="mod_lib_classe",
        )
        ...
        idx_sede = (
            sedi_esistenti.index(val_sede_corrente)
            if val_sede_corrente in sedi_esistenti
            else 0
        )
        mod_sede_sel = st.selectbox(
            "Sede",
            options=sedi_esistenti if sedi_esistenti else [""],
            index=idx_sede if sedi_esistenti else 0,
            key="mod_sel_sede",
        )
        mod_sede_libera = st.text_input(
            "O digita nuova sede (Modifica):",
            placeholder="Se non è in elenco...",
            key="mod_lib_sede",
        )
        ...
        idx_mod = (
            modalita_esistenti.index(val_mod_corrente)
            if val_mod_corrente in modalita_esistenti
            else 0
        )
        mod_modalita_sel = st.selectbox(
            "Modalità",
            options=modalita_esistenti if modalita_esistenti else [""],
            index=idx_mod if modalita_esistenti else 0,
            key="mod_sel_mod",
        )
        mod_modalita_libera = st.text_input(
            "O digita nuova modalità (Modifica):",
            placeholder="Se non è in elenco...",
            key="mod_lib_mod",
        )
        ...
        st.selectbox(
            "Modifica Avviso / Promemoria Calendar (Disabilitato)",
            options=["Funzione temporaneamente disabilitata"],
            index=0,
            disabled=True,
        )
        ...
        svolto_corrente = (
            bool(riga_corrente["Svolto"]) if "Svolto" in riga_corrente else False
        )
        mod_svolto = st.checkbox("Impegno svolto", value=svolto_corrente)

        escluso_corrente = (
            bool(riga_corrente["Escludi_Conteggio"])
            if "Escludi_Conteggio" in riga_corrente
            else False
        )
        mod_escluso = st.checkbox(
            "Escludi dal conteggio ore", value=escluso_corrente
        )

        mod_note = st.text_area("Note", value=str(riga_corrente["Note"]))

        attuale_appunto_multiplo = (
            str(riga_corrente.get("Appunto_Multiplo", ""))
            if pd.notnull(riga_corrente.get("Appunto_Multiplo", ""))
            else ""
        )
        ...
        if is_multiplo:
          ...
          opts_classi_multi = [""] + config.get("classi", [])
          for i in range(8):
            default_hi, default_mi, default_hf, default_mf, default_text = (
                9,
                0,
                10,
                0,
            ...
            idx_cls = (
                opts_classi_multi.index(def_class_val)
                if def_class_val in opts_classi_multi
                else 0
            )
            ...
            with rc1:
              m_ora_i = st.selectbox(
                  f"ModDaO{i+1}",
                  options=list(range(0, 24)),
                  index=default_hi if default_hi in range(0, 24) else 0,
                  key=f"mod_m_ora_i_{i}",
                  label_visibility="collapsed",
              )
            with rc2:
              m_min_i = st.selectbox(
                  f"ModDaM{i+1}",
                  options=list(range(0, 60)),
                  index=default_mi if default_mi in range(0, 60) else 0,
                  key=f"mod_m_min_i_{i}",
                  label_visibility="collapsed",
              )
            with rc3:
              m_ora_f = st.selectbox(
                  f"ModAO{i+1}",
                  options=list(range(0, 24)),
                  index=default_hf if default_hf in range(0, 24) else 1,
                  key=f"mod_m_ora_f_{i}",
                  label_visibility="collapsed",
              )
            with rc4:
              m_min_f = st.selectbox(
                  f"ModAM{i+1}",
                  options=list(range(0, 60)),
                  index=default_mf if default_mf in range(0, 60) else 0,
                  key=f"mod_m_min_f_{i}",
                  label_visibility="collapsed",
              )
            with rc5_sel:
              m_classe_sel = st.selectbox(
            ...
            with rc5_txt:
              m_testo_libero = st.text_input(
                  ...
                  value=def_text_val,
                  placeholder=f"O testo libero {i+1}...",
                  key=f"mod_m_testo_libero_{i}",
                  label_visibility="collapsed",
              )

            val_testo_riga = (
                m_testo_libero.strip()
                if m_testo_libero.strip()
                else m_classe_sel
            )
            if val_testo_riga:
              orario_slot_str = (
              ...
              mod_righe_multiplo_lista.append(
                  f"{orario_slot_str} {val_testo_riga}"
              )

          mod_appunto_multiplo = "\n".join(mod_righe_multiplo_lista)
        else:
          mod_appunto_multiplo = st.text_area(
              "Modifica Testo Inserito (Multi-impegno / Nota multipla)",
              value=attuale_appunto_multiplo,
              height=120,
          )
        ...
        if st.form_submit_button("Salva Modifiche", use_container_width=True):
          if mod_orario_i_str >= mod_orario_f_str:
            st.error(
                "L'orario di inizio non può essere successivo o uguale"
                " all'orario di fine."
            )
          else:
            val_ente_finale = (
                mod_ente_libero.strip() if mod_ente_libero else mod_ente_sel
            )
            val_classe_finale = (
                mod_classe_libera.strip()
                if mod_classe_libera
                else mod_classe_sel
            )
            val_sede_finale = (
                mod_sede_libera.strip() if mod_sede_libera else mod_sede_sel
            )
            val_modalita_finale = (
                mod_modalita_libera.strip()
                if mod_modalita_libera
                else mod_modalita_sel
            )
            ...
            if (
                mod_modalita_libera
                and mod_modalita_libera not in config["modalita"]
            ):
            ...
            cal_id_esistente = (
                str(df.loc[riga_idx, "Calendar_ID"])
                if "Calendar_ID" in df.columns
                else ""
            )
            if cal_id_esistente and cal_id_esistente.lower() not in [
              ...
              res_id = sincronizza_google_calendar(
                  "aggiorna", dati_evento, cal_id_esistente
              )
              df.loc[riga_idx, "Calendar_ID"] = (
                  str(res_id) if res_id else cal_id_esistente
              )
            else:
              cal_id = sincronizza_google_calendar("crea", dati_evento)
    ...
    with col_t1:
      ricerca_libera = st.text_input(
          "Ricerca libera", placeholder="Parole chiave..."
      )
    with col_t2:
      data_inizio_filtro = st.date_input(
          "Data Inizio", value=None, format="DD/MM/YYYY"
      )
    with col_t3:
      data_fine_filtro = st.date_input(
          "Data Fine", value=None, format="DD/MM/YYYY"
      )

    col_f0, col_f1, col_f2, col_f3 = st.columns(4)
    with col_f0:
      enti_disponibili = ["Tutti"] + sorted(
          df["Ente"].dropna().unique().tolist() if "Ente" in df.columns else []
      )
    ...
    with col_ord2:
      direzione_ordinamento = st.radio(
          "Direzione",
          options=[
              "Crescente (A-Z / Dal più vecchio)",
              "Decrescente (Z-A / Dal più recente)",
          ],
          horizontal=True,
      )
    ...
    if colonna_ordinamento in df_report.columns:
      if colonna_ordinamento == "Data_dt":
        df_report = df_report.sort_values(
            by=["Data_dt", "Orario Inizio"], ascending=[crescente, True]
        )
      else:
        df_report = df_report.sort_values(
            by=[colonna_ordinamento, "Data_dt"], ascending=[crescente, True]
        )

    ore_totali = df_report[df_report["Escludi_Conteggio"] != True]["Ore"].sum()

    st.success(
        f"**Risultati Report Filtrati:** {len(df_report)} attività trovate |"
        " **Totale Ore Report (escluse quelle flaggate):**"
        f" **{ore_totali:.2f} ore**"
    )
    ...
    if not df_report.empty:
      ...
      for _, row in df_report.iterrows():
        ...
        escluso_card = (
            bool(row["Escludi_Conteggio"])
            if "Escludi_Conteggio" in row
            else False
        )
        note = str(row["Note"]) if pd.notnull(row["Note"]) else ""
        appunto_mult = (
            str(row.get("Appunto_Multiplo", ""))
            if pd.notnull(row.get("Appunto_Multiplo", ""))
            else ""
        )
        ...
        badge_escluso = (
            " | <span style='color: #ff9999;'>[Escluso conteggio]</span>"
            if escluso_card
            else ""
        )
        multiplo_html = (
            "<br><em>Testo Multi-impegno:</em> <pre style='background:"
            " rgba(0,0,0,0.2); padding: 5px; border-radius: 4px; color: inherit;"
            f" white-space: pre-wrap;'>{appunto_mult}</pre>"
            if appunto_mult
            else ""
        )
      ...
      with pd.ExcelWriter(buffer_excel, engine="xlsxwriter") as writer:
        ...
        for i, col in enumerate(df_export.columns):
          lunghezza_massima = max(
              df_export[col].astype(str).map(len).max(), len(str(col))
          )
    ...
    with c_exp3:
      ...
      if pdf_data:
        ...
        st.button(
            ...
            use_container_width=True,
            help=(
                "Installa reportlab per abilitare l'esportazione PDF"
            ),
        )

    with c_exp4:
      if st.button(
          "Sincronizza eventi mancanti",
          use_container_width=True,
          help=(
              "Invia a Google Calendar gli eventi salvati che non hanno ancora"
              " un ID Calendar"
          ),
      ):
        count_sinc = 0
        ...
        if count_sinc > 0:
          salva_dati(df)
          st.success(
              f"Sincronizzati con successo {count_sinc} eventi su Google"
              " Calendar!"
          )
          st.rerun()
        else:
...
with tab4:
  ...
  with col_nav2:
    if st.button("< Mese -", use_container_width=True):
      ...
      st.rerun()
  with col_nav3:
    mese_corrente_nome = traduci_mese(
        datetime.date(
            st.session_state["cal_anno"], st.session_state["cal_mese"], 1
        ).strftime("%B")
    ).capitalize()
    st.markdown(
        f"<h3 style='text-align: center; margin: 0;'>{mese_corrente_nome}"
  ...
  df_ore_valide = (
      df_mese[df_mese["Escludi_Conteggio"] != True]
      if not df_mese.empty
      else pd.DataFrame()
  )
  ore_appuntamenti_mensili = (
      df_ore_valide["Ore"].sum() if not df_mese.empty else 0.0
  )
  ...
  if not df_mese.empty:
    for _, row in df_mese.iterrows():
      ...
      impegni_per_giorno[giorno_num].append({
          ...
          "classe": str(row.get("Classe", "")),
          "orario": (
              f"{str(row.get('Orario Inizio', ''))} -"
              f" {str(row.get('Orario Fine', ''))}"
          ),
          "ore": row.get("Ore", 0.0),
          "sede": str(row.get("Sede", "")),
  ...
  for settimana in giorni_mese:
    ...
    for giorno in settimana:
      if giorno == 0:
        ...
        bg_style = (
            "background-color: #183025; border: 1px solid #2fa866;"
            if ha_impegni
        ...
        if ha_impegni:
          ...
          dettaglio_html = (
              f"<b>Impegni del"
              f" {giorno}/{st.session_state['cal_mese']}/{st.session_state['cal_anno']}</b>"
          )
          ...
          for imp in lista_imp:
            ...
            dettaglio_html += (
                f"<div style='margin-bottom: 6px; {barrato_stile}'>"
            )
            dettaglio_html += (
                f"<b>{imp['orario']}</b> ({imp['ore']}h){escl_nota}<br>"
            )
            dettaglio_html += (
                f"<b>Ente:</b> {imp['ente']} | <b>Classe:</b>"
                f" {imp['classe']}<br>"
            )
          ...
          if len(lista_imp) == 1:
            ...
            html_righe += (
                f'<span class="badge-impegno">{testo_badge}</span>'
            )
          else:
            html_righe += (
                f'<span class="badge-impegno-multi">{len(lista_imp)}'
                ' Appuntamenti</span>'
            )

          html_righe += (
              f'<div class="tooltip-content">{dettaglio_html}</div>'
          )
          html_righe += "</div>"
  ...
  th_html = "".join(
      [f'<th class="cal-th">{gs}</th>' for gs in giorni_settimana]
  )
