...
def parse_data_italiana(val):
    if pd.isna(val) or str(val).strip() in ["", "none", "nan", "None", "NaN"]:
        return pd.NaT
    ...
    try:
        ...
        if not df_report.empty:
            ...
            for _, row in df_report.iterrows():
                e_nome = str(row.get("Ente", "")).strip() or "Non Specificato"
                c_nome = str(row.get("Classe", "")).strip() or "Non Specificata"
            ...
            t_tot.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#e2e8f0"))]))
...
def carica_dati():
    cols_standard = [
        "Data", "Mese", "Orario Inizio", "Orario Fine", "Ore", "Ente",
        "Classe", "Sede", "Modalità", "Svolto", "Escludi_Conteggio",
        "Note", "Appunto_Multiplo", "Codice_Univoco", "Calendar_ID", "Reminder_Minuti"
    ]
    ...
    try:
        ...
        df["Svolto"] = df["Svolto"].apply(lambda x: str(x).lower() in ["true", "1", "yes", "vero", "on"]) if "Svolto" in df.columns else False
        df["Escludi_Conteggio"] = df["Escludi_Conteggio"].apply(lambda x: str(x).lower() in ["true", "1", "yes", "vero", "on"]) if "Escludi_Conteggio" in df.columns else False
        df["Calendar_ID"] = df["Calendar_ID"].fillna("").astype(str) if "Calendar_ID" in df.columns else ""
        df["Reminder_Minuti"] = pd.to_numeric(df.get("Reminder_Minuti", 240), errors="coerce").fillna(240).astype(int)
...
def mostra_registri_di_classe():
    ...
    if dati:
        ...
        with c_sel1:
            classe_scelta = st.selectbox("Seleziona Classe", options=["Tutte"] + dati["classi"])
        with c_sel2:
            materia_scelta = st.selectbox("Seleziona Materia", options=["Tutte"] + dati["materie"])

        sub_tab1, sub_tab2, sub_tab3, sub_tab4 = st.tabs(["Alunni", "Presenze", "Voti", "Note e Annotazioni"])

        with sub_tab1:
            if dati["alunni"] is not None and not dati["alunni"].empty:
                df_alunni = dati["alunni"].copy()
        ...
        with sub_tab2:
            if dati["presenze"] is not None and not dati["presenze"].empty:
                df_p = dati["presenze"].copy()
                if classe_scelta != "Tutte" and "Classe" in df_p.columns:
                    df_p = df_p[df_p["Classe"].astype(str) == classe_scelta]
                st.dataframe(df_p, use_container_width=True)
            else:
                st.info("Nessuna registrazione presenze presente.")

        with sub_tab3:
            if dati["voti"] is not None and not dati["voti"].empty:
                df_v = dati["voti"].copy()
                if classe_scelta != "Tutte" and "Classe" in df_v.columns:
                    df_v = df_v[df_v["Classe"].astype(str) == classe_scelta]
                if materia_scelta != "Tutte" and "Materia" in df_v.columns:
                    df_v = df_v[df_v["Materia"].astype(str) == materia_scelta]
                st.dataframe(df_v, use_container_width=True)
            else:
                st.info("Nessun voto registrato.")

        with sub_tab4:
            if dati["note"] is not None and not dati["note"].empty:
                df_n = dati["note"].copy()
                if classe_scelta != "Tutte" and "Classe" in df_n.columns:
                    df_n = df_n[df_n["Classe"].astype(str) == classe_scelta]
    ...
    with st.form("form_orario", clear_on_submit=True):
        ...
        if tipo_inserimento == "Inserimento Multiplo":
            ...
            for i in range(8):
                ...
                val_testo_riga = m_testo_libero.strip() or m_classe_sel
                if val_testo_riga:
        ...
        if submit_button:
            if orario_inizio_str >= orario_fine_str:
                ...
                val_ente = nuovo_ente_libero.strip() or ente
                val_classe = nuova_classe_libera.strip() or classe
                val_sede = nuova_sede_libera.strip() or sede
                val_modalita = nuovo_mod_libero.strip() or modalita
                ...
                salva_dati(df)
                st.success("Attività salvata e sincronizzata!")
                st.rerun()
...
with tab2:
    ...
    def gestisci_sezione_combo(titolo_sezione, chiave_config):
        ...
        voce_selezionata = st.selectbox(
            f"Voci in {titolo_sezione}",
            options=["-- Seleziona --"] + sorted(lista_corrente),
            key=f"sel_mod_{chiave_config}",
        )

        with st.form(key=f"form_add_{chiave_config}", clear_on_submit=True):
            nuova_voce = st.text_input(f"Aggiungi a {titolo_sezione}")
            btn_aggiungi = st.form_submit_button("Aggiungi", use_container_width=True)

            if btn_aggiungi and nuova_voce.strip():
                nuova_pulita = nuova_voce.strip()
                if any(v.lower() == nuova_pulita.lower() for v in lista_corrente):
                    st.error("Voce già presente.")
                else:
        ...
        if voce_selezionata != "-- Seleziona --":
            with st.form(key=f"form_edit_{chiave_config}"):
                valore_modificato = st.text_input("Rinomina", value=voce_selezionata)
                btn_salva = st.form_submit_button("Salva")
                btn_elimina = st.form_submit_button("Elimina", type="primary")

                if btn_salva and valore_modificato.strip():
                    idx = lista_corrente.index(voce_selezionata)
                    lista_corrente[idx] = valore_modificato.strip()
                    salva_config(config)
                    st.rerun()
                if btn_elimina:
    ...
    with col1: gestisci_sezione_combo("Enti", "enti")
    with col2: gestisci_sezione_combo("Classi", "classi")
    with col3: gestisci_sezione_combo("Sedi", "sedi")
    with col4: gestisci_sezione_combo("Modalità", "modalita")

# ================= TAB 3: ARCHIVIO, MODIFICA & REPORT =================
with tab3:
    ...
    if not df.empty:
        ...
        colonne_da_nascondere = ["ID_originale", "Codice_Univoco", "Mese", "Calendar_ID", "Reminder_Minuti", "Data_dt"]
        ...
        df_editato = st.data_editor(
            ...
            hide_index=True,
            column_config={
                "Selezione": st.column_config.CheckboxColumn("Sel.", width="small"),
                "Svolto": st.column_config.CheckboxColumn("Svolto", width="small"),
                "Escludi_Conteggio": st.column_config.CheckboxColumn("Escludi", width="small"),
            },
        )
        ...
        for _, riga_ed in df_editato.iterrows():
            ...
            val_svolto = bool(riga_ed["Svolto"])
            val_escluso = bool(riga_ed["Escludi_Conteggio"])
            if df.loc[idx_orig, "Svolto"] != val_svolto or df.loc[idx_orig, "Escludi_Conteggio"] != val_escluso:
                df.loc[idx_orig, "Svolto"] = val_svolto
                df.loc[idx_orig, "Escludi_Conteggio"] = val_escluso
                modificato = True
        if modificato:
            salva_dati(df)
            st.rerun()

        righe_selezionate = df_mostra[df_editato["Selezione"] == True]["ID"].tolist() if "Selezione" in df_editato.columns else []

        col_act1, col_act2 = st.columns(2)
        with col_act1:
            if st.button("Elimina Selezionati", type="primary", use_container_width=True) and righe_selezionate:
        ...
        st.markdown("---")
        st.subheader("Generazione Report e Download")
        df_report = df.copy()
        ...
        with c_exp1:
            ...
            st.download_button("Scarica Report CSV", data=csv_data, file_name="report_attivita.csv", mime="text/csv", use_container_width=True)
        with c_exp2:
            buffer_excel = io.BytesIO()
            with pd.ExcelWriter(buffer_excel, engine="xlsxwriter") as writer:
                df_report.drop(columns=["Data_dt"], errors="ignore").to_excel(writer, index=False, sheet_name="Report")
            buffer_excel.seek(0)
            st.download_button("Scarica Excel", data=buffer_excel, file_name="report_attivita.xlsx", use_container_width=True)
        with c_exp3:
...
with tab4:
    ...
    with col_nav2:
        if st.button("< Mese -", use_container_width=True):
            ...
            st.rerun()
    with col_nav3:
        mese_nome = traduci_mese(datetime.date(st.session_state["cal_anno"], st.session_state["cal_mese"], 1).strftime("%B")).capitalize()
        st.markdown(f"<h3 style='text-align: center; margin: 0;'>{mese_nome} {st.session_state['cal_anno']}</h3>", unsafe_allow_html=True)
    with col_nav4:
        if st.button("Mese + >", use_container_width=True):
    ...
    num_appuntamenti = len(df_mese)
    ore_totali_mese = df_mese[df_mese["Escludi_Conteggio"] != True]["Ore"].sum() if not df_mese.empty else 0.0

    st.markdown(
        f"""
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
            <div style="background-color: #1c3d73; color: white; padding: 12px 20px; border-radius: 8px; flex: 1; text-align: center;">
                <span style="font-size: 14px; opacity: 0.8;">Appuntamenti Mensili</span><br>
                <span style="font-size: 22px; font-weight: bold;">{num_appuntamenti}</span>
            </div>
            <div style="background-color: #155c32; color: white; padding: 12px 20px; border-radius: 8px; flex: 1; text-align: center;">
                <span style="font-size: 14px; opacity: 0.8;">Ore Totali Valide</span><br>
                <span style="font-size: 22px; font-weight: bold;">{ore_totali_mese:.2f} h</span>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    for g in giorni_settimana:
        html_cal += f"<th class='cal-th'>{g}</th>"
    ...
    for settimana in giorni_mese:
        ...
        for giorno in settimana:
            if giorno == 0:
                html_cal += "<td class='cal-cell cal-cell-empty'></td>"
            else:
                ha_impegni = giorno in impegni_per_giorno
                bg_style = "background-color: #183025; border: 1px solid #2fa866;" if ha_impegni else "background-color: #1e293b;"
                html_cal += f"<td class='cal-cell' style='{bg_style}'><div class='day-number'>{giorno}</div>"
